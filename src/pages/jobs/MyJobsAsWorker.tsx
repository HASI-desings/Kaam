...
export default function MyJobsAsWorker() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-3">
      <h1 className="text-xl font-semibold text-text-light dark:text-text-dark">Jobs I'm working on</h1>
      {jobs.map((j) => (
        <Link
          key={j.id}
          to={j.status === "in_progress" || j.status === "assigned" ? `/jobs/${j.id}/progress` : `/jobs/${j.id}`}
      ...
      {jobs.length === 0 && (
        <p className="text-center text-sm text-text-light-secondary dark:text-text-dark-secondary py-12">No jobs yet — browse the feed to submit an offer.</p>
      )}
    </div>
  );
}
